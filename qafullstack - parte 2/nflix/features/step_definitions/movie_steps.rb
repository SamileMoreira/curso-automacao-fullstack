...
Dado("esse filme já existe no catálogo") do
...
Dado("que {string} está parte do catálogo") do |movie_code|
  steps %(
    Dado que "#{movie_code}" é um novo filme
    E esse filme já existe no catálogo
...
Então("este item deve permacener no catálogo") do
